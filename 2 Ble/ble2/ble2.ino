...
void loop() {
  ...
  if (connected && pRemoteCharacteristic != nullptr) {
    ...
    buffer[4] = (uint8_t)rssi;
    buffer[5] = PROTOCOLE_ID;
    buffer[6] = battery;
  ...
  // Attente avant nouvelle tentative (aléatoire pour éviter collisions)
  int delay_time = random(5000, 10000);  // entre 5 et 10 secondes
  delay(delay_time);
}